...
use obscuravpn_api::types::{OneExit, OneRelay, RelayPreferredExit};
...
pub struct ExitSelectionState {
    ...
    selected_cities: HashMap<(String, String), Saturating<u8>>,
    selected_countries: HashMap<String, Saturating<u8>>,
}
...
impl ExitSelectionState {
    pub fn select_next_exit<'a>(&mut self, selector: &ExitSelector, exits: &'a [OneExit], relay: &OneRelay) -> Option<&'a OneExit> {
        let selected = exits
            ...
            .max_by_key(|candidate| Self::rank(candidate, &relay.country_code, &relay.city_code, &relay.preferred_exits));
        ...
        if let Some(selected) = selected {
            ...
            *self
                .selected_cities
                .entry((selected.country_code.clone(), selected.country_code.clone()))
                .or_insert(Saturating(0)) += 1;
            *self.selected_countries.entry(selected.country_code.clone()).or_insert(Saturating(0)) += 1;
        } else {
    ...
    fn rank(
        candidate: &OneExit,
        relay_country_code: &str,
        relay_city_code: &str,
        relay_preferred_exits: &[RelayPreferredExit],
    ) -> (bool, bool, bool, u8, u32) {
        let is_preferred = relay_preferred_exits.iter().any(|e| e.id == candidate.id);
        let same_country = relay_country_code == candidate.country_code;
        let same_city = same_country && relay_city_code == candidate.city_code;
    ...
    fn exclude(&self, candidate: &OneExit) -> bool {
        self.selected_exit_ids.contains(&candidate.id)
            ...
            || self.selected_cities.get(&(candidate.country_code.clone(), candidate.city_code.clone())) >= Some(&Saturating(4))
            || self.selected_countries.get(&candidate.country_code.clone()) >= Some(&Saturating(6))
    }
}
